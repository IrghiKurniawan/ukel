...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Filter berdasarkan provinsi jika ada input
        if ($request->has('province') && $request->province) {
            $query->where('province', $request->province);
        }

        // Muat laporan dengan data user terkait
        $reports = $query->with('user')->paginate(10);
    ...
    {
        $province = auth()->user()->staff?->province; // Sesuai struktur data Anda
        return Excel::download(new NamaExport(null, $province), 'report_' . ($province ?? 'all') . '.xlsx');
    }

    public function exportByDate(Request $request)
    {
        $date = $request->input('date');
        return Excel::download(new NamaExport($date), 'report_' . ($date ?? 'all') . '.xlsx');
    }
    ...
    //     return Excel::download(new NamaExport($filterData), 'laporan.xlsx');
    // }
}
